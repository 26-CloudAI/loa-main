...
- 종목별 고유 μ/σ 로 GBM 적용
- 10~20턴마다 랜덤 뉴스 이벤트 발생 → 특정 종목 μ 변화 (5~10턴 지속)
...
class Market:
    ...
    def _try_generate_news(self) -> None:
        ...
        active_stocks = [s for s in self._cfg.stocks if s.symbol not in self._delisted]
        if not active_stocks:
            return

        spec = self._rng.choice(active_stocks)
